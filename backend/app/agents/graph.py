...
def build_graph():
    ...
    # graph.add_node("diagnosis", run_diagnosis)  # Disabled: OOM on free tier (embeddings model too large)
    graph.add_node("resolution", run_resolution)
    graph.add_node("escalation", run_escalation)

    graph.set_entry_point("router")
    # graph.add_edge("router", "diagnosis")  # Skip diagnosis to save memory
    graph.add_edge("router", "resolution")  # Go directly to resolution
